...
def wipe_database
  Tripbook::Event.map(&:destroy)
  Tripbook::Calendar.map(&:destroy)
  Tripbook::Account.map(&:destroy)
end

def authenticate(account_data)
  Tripbook::AuthenticateAccount.call(
...
def authorization(account_data)
  ...
  # { account: Tripbook::Account.first(username: account['username']),
  #   scope: AuthScope.new(token.scope) }

  { account: Tripbook::Account.first(username: account_data['username']),
    scope: AuthScope.new }
end
